...
export const paypalCheckPayment = async (paypalTransactionId: string) => {
  ...
  const resp = await verifyPayPalToken( paypalTransactionId, authToken );
  if( !resp ) return { ok: false, message: "Error al verificar el pago" };

  const { status, purchase_units } = resp;
  const { invoice_id: orderId } = purchase_units[0];

  if( status !== 'COMPLETED' ) return { ok: false, message: "Aún no se ha pagado en PayPal"};
  ...
  try {
    ...
    console.log( error );
    return { ok: false, message: "500 - No se pudo realizar" };
  }
};
...
const verifyPayPalToken = async ( paypalTransactionId: string, bearerToken: string ): Promise<PayPalOrderStatusResponse|null> => {
